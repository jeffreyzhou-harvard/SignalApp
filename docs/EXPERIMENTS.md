...
- **Gauri sampling A/B (Aug 9):** third audience @gauri__gupta (978 followers, 194 deep). Engagement-ranked sample 0.429 vs follower-ranked counterfactual 0.413 (same n, config) — ranking strategy is a wash within a small legit follower base. The 138-member intersection of both rankings scored **0.588**: the consensus core clusters best, echoing the full-depth finding. Audience is softer-structured than ishand (6 overlapping AI-research tribes, ~25% periphery). gauri-v1 stays active.
- **Gauri union (Aug 9):** deep union 251 (26% of pulled audience) collapsed HDBSCAN to k=2 (stability 1.0 — degenerate), but KMeans k=6 on identical features scored **0.712** — third algorithm inversion caught by the standing re-sweep rule. More deep data changed the right algorithm. gauri-v7-active (KMeans union) replaces gauri-v1 (0.429) as the active map. Note: her 1,000-follower pull is the newest ~18% of 5,654 — recency-biased sample, full crawl is the upgrade.
